 ...
 2) 규칙 및 점화식
   ...
   ① currentMilk == 딸기(0)이면, dp[i][j][딸기 0] = max(dp[i][j-1][이전 바나나 2] + 1, dp[i-1][j][이전 바나나 2] + 1)
      					  아니면, dp[i][j][딸기 0] = max(dp[i][j-1][딸기 0], dp[i-1][j][딸기 0])
      							  => 현재 딸기(0)를 안마심
   ② currentMilk == 초코(1)이고 이전 순서의 딸기(0)를 마셔서 dp[i][j][딸기 0] > dp[i][j][바나나 2]이면,
      				 		 dp[i][j][초코 1] = max(dp[i][j-1][이전 딸기 0] + 1, dp[i-1][j][이전 딸기 0] + 1)
      				 아니면, dp[i][j][초코 1] = max(dp[i][j-1][초코 1], dp[i-1][j][초코 1])
      				 		 => 현재 초코(1)를 안마심
   ③ currentMilk = 바나나(2)이고 이전 순서의 초코(1)를 마셔서 dp[i][j][초코 1] > dp[i][j][딸기 0]이면,
	...
	static void solution() {
		for (int i = 1; i <= n; i++) {
			for (int j = 1; j <= n; j++) {
				...
				if (currentMilk == 1 &&
						dp[i][j][0] > dp[i][j][2]) {	// 이전에 딸기(0)를 먹었는지
				...
				if (currentMilk == 2 &&
						dp[i][j][1] > dp[i][j][0]) {	// 이전에 초코(1)를 먹었는지
