...
1. 아이디어
 - 적어도 c명 영업 => c명, c+1명, ..., c+100명
   (입력: 1개 도시에서 x원으로 영업하는 최대 고객 수 = 100명)
 - 적어도 c명을 늘리기 위한 최소 금액
   => c명, c+1명, ..., c+100명 늘리는 최소 금액에서 최소값
 - 적어도 i명(i <= 입력 c)을 늘리기 위한 최소 금액
   => i명, i+1명, ..., c+100명 늘리는 최소 금액에서 최소값

 1) DP 배열 정의: int[] dp
   - dp[i]: 고객을 i명 만큼 늘릴 때, 최소 비용
   - 출력, 고객을 적어도 c명 늘릴 때의 최소 비용
     = dp[c] ~ dp[c + 100] 중, 최소값

 2) 규칙 및 점화식
   - 고객을 적어도 c명 늘릴 때의 최소 비용
     = dp[c] ~ dp[c + 100] 중, 최소값
     => 각 도시에서 홍보할 때 늘리는 고객 수 customer에 대해 (customer <= 입력 c),
        dp[customer] ~ dp[c + 100] 채워나감
