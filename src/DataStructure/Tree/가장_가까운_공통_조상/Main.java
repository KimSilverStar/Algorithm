...
1. 아이디어
 - 루트 노드 찾기
   => 트리 노드 정보 입력할 때, 자식 노드들을 체크
   => 입력 끝난 후, 체크되지 않은 노드가 루트 노드

 *** 최소 공통 조상 (Lowest Common Ancestor, LCA) 알고리즘 ***
 1) 모든 노드의 depth, 직계 부모(2^0 번째 부모) 저장
   - DFS 수행
 2) 모든 노드의 2^i 번째 부모 저장
   - DP 수행, Sparse Table 에 저장
   - int[][] parent = new int[n][21];	(n: 트리 노드 개수, 21: 트리 깊이 20이면 충분)
 ...
 3) 두 노드 n1, n2 의 depth 를 동일하게 맞춤
...
public class Main {
	...
	static int[] depth;					// DP 배열, 각 노드의 depth
	static int[][] parent;				// DP 배열, parent[nodeIdx][i]: 노드의 2^i 번째 부모

	/* DP 배열 depth[] 에 각 노드의 깊이 저장, 직계 부모(2^0 번째 부모) 저장 */
