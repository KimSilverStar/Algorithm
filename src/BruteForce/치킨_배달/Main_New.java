...
public class Main_New {
	...
	/* selectedCount: 선택한 치킨 집 개수, idx: 치킨 집 선택 시작 index */
	static void solution(int selectedCount, int idx) {
		...
		if (selectedCount == m) {
			...
			for (int i = 0; i < homeList.size(); i++) {
				...
				for (int j = 0; j < chickenList.size(); j++) {
					if (checkChickenList[j]) {
						minDistance = Math.min(
								minDistance,
								calcDistance(chickenList.get(j), homeList.get(i))
						);
					}
				}
		...
		for (int i = idx; i < chickenList.size(); i++) {
			if (!checkChickenList[i]) {
				...
				solution(selectedCount + 1,i + 1);

				// 재귀 복귀 시점: check 배열 복구
