...
- Backtracking + DFS 로 벽 3개 세울 위치 선택
- BFS 로 바이러스 영역 탐색하여 계산

1. 아이디어
 1) 추가할 벽 3개 위치 선택 => Backtracking + DFS
 2) 추가할 벽 3개 위치 선택 후,
    입력 행렬을 [0, 0] ~ [n, m] 차례로 확인
   - 칸이 바이러스(2)이면, BFS 탐색 시작
...
3. 시간 복잡도
 - DFS + Backtracking 으로 3개 벽을 추가하는 경우의 수
...
public class Main {
	...
	/* DFS + Backtracking 으로 추가할 벽 3개 위치 선택 후, 탐색 시작 */
	...
	static void bfs() {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				// 바이러스(2)이고, 아직 방문 안한 경우
				if (virusMap[i][j] == 2 && !check[i][j]) {
					check[i][j] = true;
					queue.add(new Coord(i, j));		// 탐색할 바이러스 시작 지점
					virusCount++;
				}
			}
		}
